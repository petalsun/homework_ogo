...
def train_model(model, train_loader, test_loader, epochs, learning_rate, task_second = False):
    ...
    for epoch in range(epochs):
        ...
        if epoch == epochs - 1 and task_second:
...
for name, model in models.items():
    ...
    train_losses, test_losses, train_accs, test_accs, times, inference_time = train_model(
        model, train_loader_MNIST, test_loader_MNIST, epochs_MNIST, learning_rate_MNIST, task_second=False)
    results[name] = {
...
for name, model in cifar_models.items():
    print(f'\nTraining {name}')
    train_losses, test_losses, train_accs, test_accs, times = train_model(
        model, train_loader, test_loader, epochs, learning_rate,True)
    cifar_results[name] = {
        'train_losses': train_losses,
        'test_losses': test_losses,
        'train_accs': train_accs,
